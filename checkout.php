...
$arr=array();
array_push($arr,array("item"=>"Soap","qty"=>25));
array_push($arr,array("item"=>"Shampoo","qty"=>12));

$obj=json_encode($arr, JSON_FORCE_OBJECT);
$ar=json_decode($obj,true);
...
if(isset($_POST['order'])){
    ...
    if(mysqli_num_rows($cart_query) > 0){
        while($cart_item = mysqli_fetch_assoc($cart_query)){
            ...
            array_push($items_obj,$cart_item);
        }
    }
    ...
    if($cart_total == 0){
        ...
        foreach($items_obj as $item){
            array_push($ins_obj,"($ordr_id,".$item['pid'].",".$item['quantity'].")");
        }
        $items_insrt=implode(", ",$ins_obj);

        mysqli_query($conn, "INSERT INTO `order_items` (order_id, pid, qty) VALUES $items_insrt") or die('query failed');
...
<section class="checkout">
    ...
    <form action="" method="POST">
        ...
        <div class="flex">
            ...
            <div class="inputBox">
                <span>city :</span>
                <input type="text" name="city" placeholder="e.g. erode"  required>
            </div>
            <div class="inputBox">
                <span>state :</span>
